def generate_graph_mujoco_xml(adjacency, position_dict=None, resolution=(1420, 1080)):
    """
    Generate MuJoCo XML for a graph.

    Args:
        adjacency (dict or list): Adjacency list or matrix.
        position_dict (dict): Node positions. Defaults to None.
        resolution (tuple): Resolution for the offscreen framebuffer (width, height).
    ...
    if position_dict is None:
        position_dict = {node: (i, 0, 0) for i, node in enumerate(adjacency_list)}
    ...
    xml = "<mujoco>\n"
    xml += "  <visual>\n"
    xml += f'    <global offwidth="{resolution[0]}" offheight="{resolution[1]}"/>\n'
    xml += "  </visual>\n"
    xml += "  <asset>\n"
